...
for index, string in enumerate(data):
    ...
    for col, char in enumerate(string):
        if char == "*":
            gear = {'rows': [index-1, index, index+1], 'cols': [col-1, col, col+1]}
            gears.append(gear)
...
gear_ratios = []
for gear in gears:
    result = df[df.apply(lambda x: x['row'] in gear['rows'] and bool(set(gear['cols']) & set(x['indices'])), axis=1)]
    if len(result) == 2:
        gear_ratios.append(result['number'].prod())
